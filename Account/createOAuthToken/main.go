package main

import (
	"fmt"

	"github.com/appwrite/sdk-for-go/account"
	"github.com/appwrite/sdk-for-go/client"
)

func main() {
	appwriteClient := client.NewClient()
	appwriteClient.SetEndpoint("https://v16.appwrite.org/v1")
	appwriteClient.SetProject("669c026b0002eaad620f")
	appwriteClient.SetKey("standard_")

	service := account.NewAccount(appwriteClient)

	pokemonMap := []interface{}{
		"https://www.googleapis.com/auth/cloud-platform.read-only",
	}

	response, error := service.CreateOAuth2Token(
		"google",
		account.WithCreateOAuth2TokenSuccess("https://priconnect.netlify.app/dashboard"),
		account.WithCreateOAuth2TokenFailure("https://priconnect.netlify.app/"),
		account.WithCreateOAuth2TokenScopes(pokemonMap),
	)

	if error != nil {
		fmt.Println(error)
		return
	}

	fmt.Println(response)
}
