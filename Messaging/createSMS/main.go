package main

import (
	"fmt"

	"github.com/appwrite/sdk-for-go/client"
	"github.com/appwrite/sdk-for-go/messaging"
)

func main() {
	appwriteClient := client.NewClient()
	appwriteClient.SetEndpoint("https://v16.appwrite.org/v1")
	appwriteClient.SetProject("669c026b0002eaad620f")
	appwriteClient.SetKey("standard_")

	service := messaging.NewMessaging(appwriteClient)

	pokemonMap := []interface{}{
		"test",
		"pokemonType",
		"moveset",
	}

	smsTarget := []interface{}{
		"66a1559211c9dd665124",
	}

	response, error := service.CreateSms(
		"unique()",
		"This is a message for SMS",
		messaging.WithCreateSmsTopics(pokemonMap),
		messaging.WithCreateSmsUsers(pokemonMap),
		messaging.WithCreateSmsTargets(smsTarget),
	)

	if error != nil {
		fmt.Println(error)
		return
	}

	fmt.Println(response)
}
